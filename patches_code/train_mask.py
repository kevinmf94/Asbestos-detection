...
for d in ["train", "val", "test"]:
    ...
    #MetadataCatalog.get("data_" + d).set(thing_classes=["asbestos", "builds", "streets", "greenspaces"])
    MetadataCatalog.get("data_" + d).set(thing_classes=["asbestos", "builds"])
    #MetadataCatalog.get("data_" + d).set(thing_classes=["builds", "streets", "greenspaces"])
...
cfg.SOLVER.MAX_ITER = IT  # 300 iterations seems good enough for this toy dataset; you will need to train longer for a practical dataset
...
if GENERATE_EVAL_IMAGES:
    ...
    for d in dataset:
        ...
        mask = apply_lut_mask(cv2.imread(d['file_name_mask'], -1).astype(np.uint8))
